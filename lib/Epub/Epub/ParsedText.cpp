...
bool ParsedText::hyphenateWordAtIndex(const size_t wordIndex, const int availableWidth, const GfxRenderer& renderer,
  ...
  const auto breakPoints = hyphenPoints(word);
  ...
  for (const auto& p : breakPoints) {
    if (p < 4) continue;
    const int prefixWidth = measureWordWidth(renderer, fontId, word.substr(0, p), style, true);
...
bool isVowel(char c) {
  return strchr("aeiouyäöüAEIOUYÄÖÜ", c);
}

bool isConsonant(char c) {
  return std::isalpha((unsigned char)c) && !isVowel(c);
}

std::vector<int> ParsedText::hyphenPoints(const std::string& w) {
  std::vector<int> points;
  const int minLeft = 4;
  const int minRight = 3;

  if ((int)w.size() < minLeft + minRight)
    return points;

  if (std::any_of(w.begin(), w.end(), ::isdigit) || w.find('-') != std::string::npos || w.find('@') != std::string::npos 
  || w.find('\'') != std::string::npos || strchr(",.;:!?", w.back())){
    return points;
  }

  for (int i = minLeft; i < (int)w.size() - minRight; ++i) {
    char a = std::tolower(w[i - 1]);
    char b = std::tolower(w[i]);
    ...
    // V-C
    if (isVowel(a) && isConsonant(b)) {
      points.push_back(i);
      continue;
    }

    // German ck rule
    if (i >= 2 && w[i - 2] == 'c' && w[i - 1] == 'k') {
      points.push_back(i - 1);
      continue;
    }

    // CC split with digraph protection
    if (isConsonant(a) && isConsonant(b)) {
      if (i >= 4 ){
        std::string q = w.substr(i - 4, 4);
        if (q == "lein" || q == "keit" || q == "chen" || q == "tsch" || q == "dsch")
          continue;
      if (i >= 3 ){
        std::string tri = w.substr(i - 3, 3);
        if (tri == "sch" || tri == "str" || tri == "spr" || tri == "spl" || tri == "scr" ||
            tri == "chr" || tri == "nth" || tri == "tch" || tri == "ver" || tri == "zer")
          continue;
      }
      if (i >= 2) {
        std::string dg = w.substr(i - 2, 2);
        if (dg == "ch" || dg == "ck" || dg == "th" || dg == "sh" || dg == "es" || dg == "en" || 
            dg == "er" || dg == "st" || dg == "be" || dg == "ge" || dg == "de" || dg == "te" || dg == "se")
          continue;
      }
      }
      points.push_back(i);
